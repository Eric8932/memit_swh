...
def compute_z(
    ...
    只看subject token的最后一个在critical layer的最后一层，能否预测出object(target)
    """
    ...
    for i in range(len(rewriting_prompts)):
        ex_len = input_tok["attention_mask"][i].sum()#每个prompt的有效长度
        rewriting_targets[i, ex_len - len(target_ids) : ex_len] = target_ids#要预测的部分赋真正的id，其他部分都不用预测

    # Compute indices of the tokens where the fact is looked up
    lookup_idxs = [
        find_fact_lookup_idx(#找到subject最后一个token在prompt中的位置
    ...
    loss_layer = max(hparams.v_loss_layer, layer)#不能超过模型的最高层
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        if cur_layer == hparams.layer_module_tmp.format(layer):
            ...
            # Add intervened delta--同一事实不同prompt加的是同样的delta
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        with nethook.TraceDict(
            module=model,
            layers=[
                hparams.layer_module_tmp.format(loss_layer),#loss_layer是最后一层，因为要得到最后一层的输出
            ...
            #在trace底下前向传播的模型，会在layers做edit_output
            ...
            kl_logits = torch.stack(
                [
                    ...
                    for i, idx in enumerate(lookup_idxs[-len(kl_prompts) :])#只看最后一个prompt的subject最后一个位置，用它来算kl_loss
                ],
                dim=0,
            )
        ...
        #利用tr，把对应层的激活值改变，然后重新计算模型对于所有位置的输出。只对rewriting_prompts做
        full_repr = tr[hparams.layer_module_tmp.format(loss_layer)].output[0][
            : len(rewriting_prompts)
        ]
        log_probs = torch.log_softmax(ln_f(full_repr) @ lm_w + lm_b, dim=2)#这已经是最后一层的输出了
        loss = torch.gather(
            log_probs,
            2,
        ...
        nll_loss_each = -(loss * mask).sum(1) / target_ids.size(0)#target长度在不同prompt一致
...
def find_fact_lookup_idx(
    ...
    if fact_token_strategy == "last":
        ...
        ret = repr_tools.get_words_idxs_in_templates(#在prompt中，subject的word idx
            tok=tok,
