...
def compute_ks(
    ...
    layer_ks = get_module_input_output_at_words(#得到不同request 不同prefix 对应层的输入和输出
        model,
        tok,
        layer,
        context_templates=[
    ...
    ans = []
    for i in range(0, layer_ks.size(0), context_len):#不同的request的模版数量一致。针对不同的request
        tmp = []
        for j in range(len(context_type_csum) - 1):#针对不同模版内取平均
            start, end = context_type_csum[j], context_type_csum[j + 1]
            tmp.append(layer_ks[i + start : i + end].mean(0))
        ans.append(torch.stack(tmp, 0).mean(0))#针对不同模版再取平均
